...
var firebaseConfig = {
  ...
  function setupRealTimeListener() {
    dataCollection.orderBy("date", "desc").onSnapshot(
      (snapshot) => {
  ...
  function saveData() {
    var formData = {
      ...
      amount: Number.parseFloat(document.getElementById("amount").value.replace(/[^0-9.-]+/g, "")), // Parse amount as float
  ...
  function sortTableByBrand() {
    var input, filter, table, tr, td, i, txtValue
    input = document.getElementById("brandSortInput")
    ...
    for (i = 0; i < tr.length; i++) {
      td = tr[i].getElementsByTagName("td")[4] // Brand is the 5th column (index 4)
      if (td) {
  ...
  // Add event listener to the brand sort input
  document.addEventListener("DOMContentLoaded", () => {
    var brandSortInput = document.getElementById("brandSortInput")
    if (brandSortInput) {
      brandSortInput.addEventListener("input", sortTableByBrand)
    }
  })
  ...
  function exportPDF(
    customHeader = "BUDGETWISE FREE INSTALLATION",
    customFooter = "MI&I REFRIGERATION AND AIRCONDITIONING SERVICES",
    excludeIndexes = [10, 11],
  ) {
      ...
      const rowData = Array.from(row.querySelectorAll("td"))
        .map((td, index) => {
          if (excludeIndexes.includes(index)) {
            ...
            if (index === 8) {
              // Assuming index 4 is the amount
              const amount = Number.parseFloat(cleanedText)
              if (!isNaN(amount)) {
